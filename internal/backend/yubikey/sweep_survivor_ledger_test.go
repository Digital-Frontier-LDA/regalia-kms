...
var pivDriverGoOperands = []survivorEntry{
	...
	{line: 96, operand: 1, status: closedFalseDirectionOnly, closure: "TestReadyRefusesACancelledContextEvenWhenCardsArePresent", whyTrueOpen: "TRUE-direction test would assert Ready=true on a live context with cards; the test only asserts Ready=false on a cancelled context", site: "ctx.Err() != nil"},

	// piv_driver.go:100 — `return err == nil && len(cards) > 0` (Ready)
	{line: 100, operand: 0, status: closedFalseDirectionOnly, closure: "TestReadyReportsTruthfullyAcrossBothHalvesOfItsReturn/not_ready_when_pivCards_errors", whyTrueOpen: "the test subtest `ready_when_cards_are_present` does assert Ready=true on a healthy pivCards, which catches the FALSE-direction's over-refusal — but it does not isolate the :100 op0 operand from :100 op1, so the assertion is on the conjunction, not the operand", site: "err == nil"},
	{line: 100, operand: 1, status: closedFalseDirectionOnly, closure: "TestReadyReportsTruthfullyAcrossBothHalvesOfItsReturn/not_ready_when_no_cards_are_present", whyTrueOpen: "TRUE-direction test would assert Ready=true on (cards, nil); the seam returns (nil, nil) for this subtest, but that does not falsify the operand — it falsifies the conjunction", site: "len(cards) > 0"},

	// piv_driver.go:112 — Identity's usable check
	{line: 112, operand: 0, status: closedByTest, closure: "TestASessionThatIsNotUsableRefusesWithoutReachingTheCard/Identity", site: "session.usable(ctx); err != nil"},

	// piv_driver.go:116 — Identity's serial check
	{line: 116, operand: 0, status: recordedMasked, closure: "card.Serial() panics on a nil-handed card before returning — :116 op0 is unreachable without a Serial() seam", masked: "the nil-handed card", site: "err != nil || strconv.FormatUint(uint64(serial), 10) != session.serial"},
	{line: 116, operand: 1, status: recordedMasked, closure: "same as :116 op0", masked: "the nil-handed card", site: "err != nil || strconv.FormatUint(uint64(serial), 10) != session.serial"},

	// piv_driver.go:123 — Policies' usable check
	{line: 123, operand: 0, status: closedByTest, closure: "TestASessionThatIsNotUsableRefusesWithoutReachingTheCard/Policies", site: "session.usable(ctx); err != nil"},

	// piv_driver.go:127 — Policies' parseSlot check
	{line: 127, operand: 0, status: closedByTest, closure: "TestAnObjectIDThatIsNotASlotNeverReachesTheCard/Policies", site: "if err != nil {"},

	// piv_driver.go:131 — Policies' KeyInfo error
	{line: 131, operand: 0, status: recordedMasked, closure: "KeyInfo() panics on a nil-handed card before returning", masked: "the nil-handed card", site: "if err != nil {"},

	// piv_driver.go:138 — PINRetries' usable check
	{line: 138, operand: 0, status: closedByTest, closure: "TestASessionThatIsNotUsableRefusesWithoutReachingTheCard/PINRetries", site: "session.usable(ctx); err != nil"},

	// piv_driver.go:142 — PINRetries' Retries error
	{line: 142, operand: 0, status: recordedMasked, closure: "Retries() panics on a nil-handed card before returning", masked: "the nil-handed card", site: "if err != nil {"},

	// piv_driver.go:149 — Login's usable+len chain
	{line: 149, operand: 0, status: closedFalseDirectionOnly, closure: "TestLoginRefusesBeforeVerifyingOnASessionThatIsNotUsable", whyTrueOpen: "TRUE-direction test would assert Login succeeds on a usable session — needs a VerifyPIN seam; :149 op0 is the usable check and the seam fixture does not provide a session that is usable AND has VerifyPIN succeed", site: "session.usable(ctx); err != nil"},
	{line: 149, operand: 1, status: closedByTest, closure: "TestALoginPINBelowTheAcceptedLengthNeverReachesTheCard", site: "len(pin) < 6"},
	{line: 149, operand: 2, status: recordedMasked, closure: "piv-go's encodePIN refuses a PIN longer than 8 bytes before any transmission, so :149 op2 cannot be the sole refuser", masked: "piv-go's encodePIN", site: "len(pin) > 64"},

	// piv_driver.go:164 — Sign's privateKey error
	{line: 164, operand: 0, status: recordedMasked, closure: "privateKey returns (nil, zero, ErrUnavailable) under any failure mode reachable in this process; Sign with key=nil falls through to :168 (signer, ok := key.(crypto.Signer) → ok=false), and :168 catches it without :164", masked: "piv_driver.go:168[0]", site: "if err != nil {"},

	// piv_driver.go:168 — Sign's !ok signer
	{line: 168, operand: 0, status: recordedMasked, closure: ":168 fires only if key.(crypto.Signer) returns ok=false, and the only path that reaches it without :164 firing is when privateKey returned (nil, zero, nil) — which :164 alone does not produce", masked: "piv_driver.go:164[0]", site: "if !ok {"},

	// piv_driver.go:172 — Sign's !valid + !algorithmMatches
