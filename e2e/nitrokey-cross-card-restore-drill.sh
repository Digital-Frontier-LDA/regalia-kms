...
initialise(){ local card="$1" r; r="$(reader "$card")"; [ -n "$r" ] || die "cannot resolve $card to a reader"
  sc-hsm-tool --reader "$r" --initialize --so-pin "$(pin_of "$card" so)" --pin "$(pin_of "$card" user)" \
    --dkek-shares 1 --label regalia-xcard-drill >>"$LOG" 2>&1 || die "initialise $card failed"; }
import_share(){ local card="$1" r; r="$(reader "$card")"
  sc-hsm-tool --reader "$r" --import-dkek-share "$STATE/dkek.pbe" --password "$DKEK_PW" > "$STATE/import-$card.log" 2>&1 \
...
keys="$(pkcs15-tool --reader "$(reader "$SRC")" --list-keys 2>/dev/null || true)"
KEY_REF="$(awk -v want="$KEY_ID" '
    /^Private/                 { ref = "" }
    /^[ \t]*Key ref[ \t]*:/    { ref = $4 }
    /^[ \t]*ID[ \t]*:/         { if ($NF == want && ref != "") { print ref; exit } }' <<< "$keys")"
[ -n "$KEY_REF" ] || die "cannot find the key reference of ID $KEY_ID"
sc-hsm-tool --reader "$(reader "$SRC")" --wrap-key "$STATE/kek.wrapped" --key-reference "$KEY_REF" --pin "$SRC_USER_PIN" >>"$LOG" 2>&1 \
...
sc-hsm-tool --reader "$(reader "$DST")" --unwrap-key "$STATE/kek.wrapped" --key-reference "$KEY_REF" --pin "$DST_USER_PIN" >>"$LOG" 2>&1 || true
...
rm -f "$STATE/kek.wrapped" "$STATE/dkek.pbe" "$STATE"/import-*.log
say "transcript: $LOG"
